...
live system), Phase B (LLM-diversified link universe), Phase 3's agent fleet
(Fable-5 Master digest, expression engine, link validation, News/Catalyst +
Risk/Borrow agents), and Phase 4 (delivery: dashboard + basement-independent
weekly email) — the forward paper-trade is running (`track.py`). **Remaining:**
...
- **Phase 3 — Daily pipeline + agents.** ✅ **Built** (agent fleet complete) — the **Master/Orchestrator digest** (`elp/digest.py`, `digest.py`, Fable-5 with Opus-4.8 fallback), the **expression engine** (`elp/express.py`: paired two-legged long/short ideas, liquidity-chosen), **link validation** (`elp/linkcheck.py`, the deterministic form of the QA/verification agent), the **News/Catalyst agent** (`elp/news.py`, `elp/catalyst.py`: a 3-source RSS + Tiingo + web-search Opus ensemble reconciled by a master, soft-derating `none`/confounded ideas), and the **Risk/Borrow agent** (`elp/risk.py`, `risk.py`: hybrid — deterministic borrow proxy / earnings-window / liquidity facts + a thin Opus note, soft-derating hard-borrow / post-earnings / thin ideas and flagging "short via options" for hard-to-borrow shorts). All fail-soft, recommendations only; the digest ranks over every agent's verdict. *Verified:* live smokes flagged a confounded name, confirmed a real customer-earnings catalyst, and borrow-checked long ideas' short legs.
...
- **Phase 5 — Paper-trading validation.** 🟨 **Running** — OOS clock started 2026-07-04 (`paper_start.txt`); accruing net-of-cost P&L. **Kill rule (set 2026-07-05):** pass = net Sharpe ≥ 0.5 **and** positive net expectancy **and** ≥5 ideas/month, judged at the later of 12 months and ≥30 closed trades (full definition in §11.8); any miss → stop. This is how we answer the net-of-cost question the literature couldn't.
...
**Phase-5 kill rule (SET 2026-07-05 with Pierre).** The forward paper-trade PASSES — the sole gate that unlocks Phase 6 — only if, judged at the **later of** (a) 12 months after `paper_start` (**2026-07-04**, so ~2027-07-04) and (b) **≥30 closed OOS trades**, **all three** hold: (1) **net Sharpe ≥ 0.5** (annualized from the closed-trade net-return series); (2) **positive net expectancy** (mean net return per closed trade > 0, after costs); and (3) **≥5 ideas/month** average dealflow over the window (thin flow is itself a fail — a strategy that can't source trades isn't deployable). Any miss → **FAIL → stop**: the options overlay is cancelled, not "tried anyway." Honest prior: the literature says this edge is likely weak, so FAIL is the base case to disprove, not a formality.
